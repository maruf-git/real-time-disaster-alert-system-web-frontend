...
import { ShieldCheck, Activity, RefreshCw, Volume2, VolumeX, MapPin, Droplets, Wind, Thermometer, CloudRain, Search, Filter } from 'lucide-react';
...
export default function Home() {
    ...
    useEffect(() => {
        setCurrentPage(1);
    }, [selectedLocationId, selectedDisasterId, selectedSeverity, selectedStatus, searchQuery]);
    ...
    const filteredAlerts = alerts.filter(alert => {
        // 1. Location Filter
        if (selectedLocationId !== 'all' && alert.location_id != selectedLocationId) return false;

        // 2. Disaster Filter
        const disasterIdStr = alert.disaster_id?.toString() || '';
        if (selectedDisasterId !== 'all' && disasterIdStr !== selectedDisasterId) return false;
        ...
        if (searchQuery.trim() !== '') {
            ...
            const disasterMatch = (alert.disaster_name || '').toLowerCase().includes(query);
    ...
    const showDangerTheme = hasActiveAlert && !emergencyDismissed && selectedLocationId !== 'all';
    ...
    useEffect(() => {
        if (hasActiveAlert && sirenEnabled && !sirenMuted && !emergencyDismissed && selectedLocationId !== 'all') {
            if (audioRef.current) {
                ...
                if (playPromise !== undefined) {
                    playPromise
                        ...
                        .catch(e => {
                            console.warn("Audio autoplay blocked by browser policy. User interaction needed.", e);
    ...
    }, [hasActiveAlert, sirenEnabled, sirenMuted, emergencyDismissed, selectedLocationId]);
                        ...
                        <div className="bg-gray-950 border-2 border-red-500 rounded-2xl shadow-2xl shadow-red-900/60 w-full max-w-lg overflow-hidden">
                            ...
                            <div className="bg-red-600 px-6 py-4 flex items-center gap-3">
                                ...
                                <div className="flex-1">
                                    <p className="text-white font-black text-lg uppercase tracking-widest">⚠ ACTIVE DISASTER ALERT</p>
                                    <p className="text-red-100 text-xs">{activeAlerts.length} active alert{activeAlerts.length !== 1 ? 's' : ''} detected · {lastUpdated?.toLocaleTimeString()}</p>
                                </div>
                                {audioPlaying && (
                            ...
                            <div className="p-5 space-y-3 max-h-64 overflow-y-auto">
                                {activeAlerts.slice(0, 3).map(alert => (
                                    <div key={alert.id} className="flex items-start gap-3 p-3 bg-red-950/60 border border-red-800 rounded-xl">
                                        <div className="w-2 h-2 rounded-full bg-red-400 animate-pulse mt-1.5 flex-shrink-0" />
                                        <div className="flex-1 min-w-0">
                                            <p className="text-white font-bold text-sm truncate">{alert.title || alert.disaster_name}</p>
                                            <p className="text-red-300 text-xs">{alert.location_name} · {alert.severity_level || alert.severity || 'Alert'}</p>
                                            {alert.description && <p className="text-red-200/70 text-xs mt-0.5 line-clamp-2">{alert.description}</p>}
                                        </div>
                                        <button
                                ...
                                {activeAlerts.length > 3 && (
                                    <p className="text-red-400 text-xs text-center">+{activeAlerts.length - 3} more alert{activeAlerts.length - 3 !== 1 ? 's' : ''}</p>
                                )}
                            </div>
                            ...
                            <div className="px-5 pb-5 flex items-center gap-3">
                                <button
                                    onClick={toggleSiren}
                                    className={`flex items-center gap-2 px-3 py-2 rounded-lg border text-sm font-semibold transition ${sirenEnabled ? 'bg-red-700 border-red-600 text-white hover:bg-red-800' : 'bg-gray-800 border-gray-600 text-gray-400 hover:bg-gray-700'
                                        }`}
                                >
                                    {sirenEnabled ? <Volume2 className="w-4 h-4" /> : <VolumeX className="w-4 h-4" />}
            ...
            <div className="text-white py-16 px-4 sm:px-6 lg:px-8 relative overflow-hidden bg-slate-900">
                ...
                <div className="max-w-7xl mx-auto relative z-10">
                    <div className="md:flex justify-between items-end mb-8">
                        <div>
                            <h1 className="text-4xl md:text-5xl font-extrabold tracking-tight mb-4">
                                National Disaster <br className="hidden md:block" />
                                <span className="text-transparent bg-clip-text bg-gradient-to-r from-red-400 to-orange-400">
                                    Monitoring System
                                </span>
                            </h1>
                            <p className="max-w-xl text-lg text-slate-400">
                                Real-time AI-powered weather analysis and automated alert dispatch for Bangladesh.
                            </p>
                        </div>
                        ...
                        <div className="mt-6 md:mt-0 flex flex-col items-end gap-4">
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                <button
                                    onClick={toggleSiren}
                                    className={`flex items-center gap-2 px-3 py-1.5 rounded-lg border transition ${sirenEnabled
                                ...
                                >
                                    {sirenEnabled ? <Volume2 className="w-4 h-4" /> : <VolumeX className="w-4 h-4" />}
                                    <span className="text-sm font-medium hidden sm:inline">Siren {sirenEnabled ? 'ON' : 'OFF'}</span>
                                </button>
                                ...
                                <div className="flex items-center gap-2 text-sm mb-1">
                                    {showDangerTheme ? (
                                        <>
                                            ...
                                            <span className="text-white font-bold uppercase tracking-wider">Active Threat Detected</span>
                                        </>
                                    ) : (
                                        <>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-8">
                        {[
                            { label: 'Active Alerts', value: activeAlerts.length, color: 'text-red-400' },
                            { label: 'Monitored Regions', value: locations.length || '8', color: 'text-blue-400' },
                            { label: 'System Uptime', value: '99.9%', color: 'text-green-400' },
                        ].map((stat, i) => (
                            <div key={i} className="backdrop-blur border p-4 rounded-xl bg-slate-800/50 border-slate-700">
                                <div className={`text-2xl font-bold ${stat.color}`}>{stat.value}</div>
                                <div className="text-xs uppercase tracking-wider font-medium text-slate-400">{stat.label}</div>
                            </div>
                        ))}
                    </div>
                </div>
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 -mt-8 relative z-20">
                ...
                {selectedLocationId !== 'all' && (
                    <div className="mb-6">
                        {weatherLoading ? (
                            ...
                            <div className={`rounded-2xl shadow-lg border p-5 text-white ${showDangerTheme
                                ...
                                <div className="flex items-center justify-between mb-3">
                                    <div className="flex items-center gap-2">
                                        ...
                                        <span className="text-sm font-semibold uppercase tracking-wider opacity-80">
                                            Live Weather — {locations.find(l => l.id == selectedLocationId)?.name}
                                        </span>
                                    </div>
                                    <span className="text-xs opacity-50">
                                        {weather.time ? new Date(weather.time).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : 'Now'}
                                    </span>
                                </div>
                                {/* Row 1: Weather Stats */}
                                <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-4">
                                    ...
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            <div className="text-2xl font-bold">{weather.rain ?? '—'} mm</div>
                                ...
                                <div className="border-t border-white/10 pt-4 mt-2 grid grid-cols-1 sm:grid-cols-2 gap-4">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div>
                                            <div className="text-xl font-bold">{weather.aqi !== undefined ? weather.aqi : '—'}</div>
                                            <div className="text-xs opacity-60">Air Quality Index (US)</div>
                                        </div>
                                    </div>
                                    <div className="flex items-center gap-3">
                                        <div className="p-2 bg-white/10 rounded-lg">
                ...
                {selectedLocationId === 'all' && (
                    <div className="mb-6 bg-white rounded-2xl shadow-sm border border-dashed border-slate-200 p-5 text-center text-slate-400">
                        ...
                        <p className="text-sm">Select a specific location to view real-time weather conditions.</p>
                    </div>
                )}
                    ...
                    <div className="flex justify-between items-center mb-8 border-b border-slate-100 pb-4">
                        <div className="flex items-center gap-4">
                            <h2 className="text-2xl font-bold text-slate-800 flex items-center gap-2">
                                ...
                                {selectedLocationId === 'all' ? 'Live Alert Feed' : 'Regional Alerts'}
                            </h2>
                            {/* Audio Indicator */}
                            {audioPlaying && (
                    ...
                    {loading && alerts.length === 0 ? (
                        ...
                        <div className="flex flex-col items-center justify-center py-20 text-center">
                            ...
                            <h3 className="text-xl font-bold text-slate-800 mb-2">
                                {selectedLocationId === 'all' ? 'All Clear' : 'No Alerts in this Region'}
                            </h3>
                            <p className="text-slate-500 max-w-sm">
                                No disaster alerts are currently active{selectedLocationId !== 'all' && ' for the selected location'}.
