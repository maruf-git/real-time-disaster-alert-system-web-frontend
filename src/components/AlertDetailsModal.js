...
export default function AlertDetailsModal({ alert, onClose }) {
  ...
  const getIcon = (type) => {
    switch (type?.toLowerCase()) {
      ...
      case "cyclone":
        return <Wind className="w-10 h-10 text-cyan-600" />;
      case "heatwave":
        return <Thermometer className="w-10 h-10 text-orange-600" />;
      default:
          ...
          <div className="flex items-center gap-4 mt-4">
            ...
            <div>
              ...
              <div className="flex items-center gap-2 mt-1">
                <span
                  className={`px-2.5 py-0.5 rounded-full text-xs font-bold uppercase tracking-wide border ${getSeverityColor(alert.severity)}`}
                >
